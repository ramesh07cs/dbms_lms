...
export default function BorrowHistory() {
    ...
    return (
        <>
            <h2 className="text-2xl font-bold text-slate-800 mb-6">Borrow History</h2>
                ...
                <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="text-left px-6 py-3 text-sm font-medium text-slate-700">Borrow Date</th>
                            ...
                            <th className="text-left px-6 py-3 text-sm font-medium text-slate-700">Fine Amount</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((b) => (
                            <tr key={b.borrow_id} className="border-b border-slate-100">
                                ...
                                <td className="px-6 py-4">{b.issue_date ? new Date(b.issue_date).toLocaleDateString() : '-'}</td>
                                <td className="px-6 py-4">{b.due_date ? new Date(b.due_date).toLocaleDateString() : '-'}</td>
                                <td className="px-6 py-4">{b.return_date ? new Date(b.return_date).toLocaleDateString() : '-'}</td>
                                <td className="px-6 py-4">
                                    <span className={b.borrow_status === 'ACTIVE' ? 'text-amber-600' : b.borrow_status === 'PENDING' ? 'text-slate-500' : 'text-green-600'}>{b.borrow_status}</span>
                                </td>
                                <td className="px-6 py-4">{b.fine_amount != null && b.fine_amount > 0 ? `Rs ${b.fine_amount}` : '-'}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
